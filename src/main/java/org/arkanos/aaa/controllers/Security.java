...
public class Security {
	 ...
	 */
	static public TokenInfo authenticateToken(HttpServletRequest request) throws IOException {
	...
	static public boolean checkResetKey(String secret_key, String password, String reset_key) {
		int i = 0;
		if ((secret_key == null) || (secret_key.length() != reset_key.length()))
			return false;
	 ...
	 *             whenever problems occur reading cookies.
	 */
	static public void invalidateToken(HttpServletRequest request) throws IOException {
		TokenInfo token_info = null;
		for (Cookie c : request.getCookies()) {
			if (c.getName().compareTo(Security.TOKEN_COOKIE_NAME) == 0) {
