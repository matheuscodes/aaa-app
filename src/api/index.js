...
var API = {
  ...
  placeSeason: function(season, success) {
    ...
    xmlhttp.onreadystatechange = function() {
		    if (xmlhttp.readyState == 4) {
		    	if (xmlhttp.status == 201) {
			    	var download = JSON.parse(String(xmlhttp.responseText));
...
var User = {
  ...
  processLogin: function(callback) {
    ...
    if (document.getElementById("password")) {
      ...
      content.hashed_password = String(CryptoJS.MD5(password));
    }
  ...
  pushTrainingDraft: function(distance, type, count) {
    ...
    if (!this.training_draft[String(type)]) this.training_draft[String(type)] = {};
    this.training_draft[String(type)][String(distance)] = count;
  },
